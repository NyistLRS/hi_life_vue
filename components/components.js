...
var goodsDetailTpl = {
	...
	template:'<div class="goods-detail">'
			+'<header :class="{fixdStyle:isTop}"><span v-if="isTop">茶点叉烧包</span><i class="goods-back"></i><span class="good-oper"><i class="goods-share"></i><i class="goods-collection"></i></span></header>'
			+'<section v-if="isfixed" class="good-fixed-detail"><small><span class="good-price">¥18.89</span><span class="good-sales">已售981件</span></small></section>'
			+'<section class="goods-detail-contanier" @touchmove="move" ref="aa">'
	...
	methods:{
		move:function(e){
			...
			console.log(a);
			if(this.$refs.aa.scrollTop>=119){
				this.isTop = true;
			}else{
				this.isTop = false;
			}
			if(this.$refs.aa.scrollTop>=159){
				this.isfixed = true;
			}else{
				this.isfixed = false;
			}
